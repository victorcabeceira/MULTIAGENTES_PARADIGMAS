	...
	private static void createCombats() throws StaleProxyException {
		...
		emperor.start();
			...
			for (int i = 1; i <= numGladiator/2; i++) {
				round[i][0] = container.createNewAgent("Gladiator0"+Integer.toString(gladiator+1), "agents.Gladiator", new Object[]{
					"Gladiator0"+Integer.toString(gladiator+2)});
				round[i][0].start();
				round[i][1] = container.createNewAgent("Gladiator0"+Integer.toString(gladiator+2), "agents.Gladiator", new Object[]{
					"Gladiator0"+Integer.toString(gladiator+1)});
				round[i][1].start();			
